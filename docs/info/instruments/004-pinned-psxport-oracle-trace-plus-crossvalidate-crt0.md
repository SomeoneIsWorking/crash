...
The pinned psxport `ad5cf802` `oracle_trace` plus `crossvalidate_crt0.py`, as wired by each title's CMake
oracle boundary target for its in-image libcInit shape.

## Validated by

The independent CPU fixture passed 39/39. Real verified `SCUS_949.00`, `SCUS_941.54`, and
`SCUS_942.44` traces independently captured each first executed crt0 jal at steps 57,910, 81,725, and
71,790 respectively; each crossvalidation agreed on 6/6 fields. The bounded crossvalidator selftest
