...
| Differential harness | 🟡 first recompiled boundaries | CMake `crash1_oracle_boot_check`, `crash1_recomp_boundary_check`; `tools/crash1_recomp.py`; framework oracle tools | Symbolic/oracle crt0 remains 6/6; canonical oracle ordinal capture and the generated port agree 34/34 at calls one and two. Out-of-text emission, a short 1/2 call window and an altered second-call register prove refusal/disagreement. No port-side per-instruction lockstep yet |
