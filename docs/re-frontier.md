...
- status: re-partial
- deps: CRASH1-03
- evidence: The verified USA executable and deliberately empty title seed additions produced 115 static candidate seeds and 653 emitted candidates with zero configured overlays under pinned psxport `7f5d3f13`, emitter version `2026-08-12.1`. A focused port runner with the direct Crash 1 runtime installed executed entry `0x8003E018` through eight distinct call targets: `0x80011A18`, `0x80011D88`, `0x8003E0C0`, `0x8001652C`, `0x8003F224`, `0x80042B1C`, `0x8004319C`, and `0x8003E1F8`. Against the independent Mednafen CPU oracle's canonical ordinal captures, `pc`, all 31 nonzero GPRs, `lo`, and `hi` agreed 34/34 at steps 57,910, 57,931, 57,935, 57,962, 62,065, 62,073, 62,077, and 62,081. Two instructions after call eight, the oracle leaves mapped executable text at the BIOS exception vector `0xBFC00180`. The shared oracle selftest passed 8/8. Crash's real trace capped immediately before call eight refused with a `7/8` denominator and no boundary block. The production emitter refused an out-of-text seed, and the production comparator reported exactly one named disagreement after its eighth-boundary port `a0` was changed.
...
- gap: Equality is proven only from the executable entry through its eighth executed call. The next observed boundary is the transition to `0xBFC00180`; model and differentially verify the BIOS/exception path before advancing later boot. BIOS, hardware and 645 unexecuted emitted bodies are not certified. Of 653 emitted candidates, only nine addresses have execution provenance (eight generated bodies executed and one next target observed). Static table discovery also emits data-like fragments (issue #3), so neither the 115-seed nor 653-candidate count is a correctness denominator.
- notes: Generated code is sacrosanct. The boundary runner installs a direct `Crash1Runtime`; both legacy views remain null and its native-boot method refuses rather than jumping past the unresolved BIOS transition. Keep explicit title seed additions empty until a real fail-fast indirect miss supplies an address and rationale; never copy another game's set.
