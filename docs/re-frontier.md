...
- status: re-verified
- deps: CRASH1-02
- evidence: Against pinned psxport `9f1bb927`, the independent CPU fixture passed 39/39 positive, hardware-stop negative, instruction-stepping, RAM-mirroring, and modeled-return checks. `oracle_trace` independently captured the first real crt0 call at step 57,910, target `0x80011A18`; `crossvalidate_crt0.py` compared 6 fields and agreed 6/6 on gp, call target, a0, planned sp, planned a0, and planned a1. A 50,000-step control reached no call and refused with exit 2, zero comparisons. CMake `crash1_oracle_boot_check` reproduces provisioning, the fixture, and the real comparison from one asset-gated target.
...
- status: re-partial
- deps: CRASH1-03
- evidence: The verified USA executable and deliberately empty title seed additions produced 115 static candidate seeds and 653 emitted candidates with zero configured overlays under pinned psxport `9f1bb927`, emitter version `2026-08-12.1`. A focused port runner executed entry `0x8003E018` through the leaf initializer at first call `0x80011A18` to the independently observed second call `0x80011D88`. Against the independent Mednafen CPU oracle's canonical ordinal captures, `pc`, all 31 nonzero GPRs, `lo`, and `hi` agreed 34/34 at step 57,910 and 34/34 at step 57,931. The shared oracle selftest passed 8/8, including first-call alias equality, a complete 33-register block, distinct call-one/call-two targets, and missing-call-three refusal; Crash's real trace capped immediately before call two likewise refused with a `1/2` denominator and no boundary block. The production emitter refused an out-of-text seed, and the production comparator reported exactly one named disagreement after its second-boundary port `a0` was changed.
...
- gap: Equality is proven only from the executable entry through its second executed call. Resume inside `0x80011D88` at its next deterministic call or hardware boundary and narrow the first real divergence; BIOS, hardware and 651 unexecuted emitted bodies are not certified. Of 653 emitted candidates, only three addresses have execution provenance (two generated bodies executed and one next target observed). Static table discovery also emits data-like fragments (issue #3), so neither the 115-seed nor 653-candidate count is a correctness denominator.
